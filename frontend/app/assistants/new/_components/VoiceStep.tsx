...
const VoiceStep: React.FC<AssistantStepProps> = ({ form }) => {
  ...
  const [availableVoices, setAvailableVoices] = useState(VOICES[selectedVoiceProvider] || []);
  const [selectedVoice, setSelectedVoice] = useState<string | null>(form.getFieldValue('voiceId') || null);
    ...
    <div className="wizard-form-section">
      ...
      >
        <Paragraph style={{ marginBottom: '16px' }}>
          Choisissez le service qui produira la voix de votre assistant lors des appels téléphoniques.
        </Paragraph>
        
        <Form.Item 
          name="voiceProvider" 
          rules={[{ required: true, message: "Le fournisseur de voix est requis" }]}
        >
          <Select 
            size="large"
      ...
      >
        <Paragraph style={{ marginBottom: '16px' }}>
          Sélectionnez la voix qui définira comment votre assistant sonnera lors des appels.
        </Paragraph>
      ...
      >
        <Paragraph style={{ marginBottom: '16px' }}>
          Écoutez un exemple de la voix sélectionnée pour vous assurer qu&apos;elle correspond à l&apos;identité de votre assistant.
        </Paragraph>
        ...
        <div style={{ padding: '20px 0' }}>
          <Button 
            ...
            disabled={!canPreview}
            style={canPreview ? { boxShadow: '0 0 10px rgba(119, 69, 255, 0.5)' } : {}}
          />
          
          <Paragraph style={{ marginTop: '12px', opacity: canPreview ? 1 : 0.6 }}>
            {canPreview 
