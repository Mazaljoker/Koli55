...
import {   Card, Title, Text, Grid, Col, Flex, Metric,   List, ListItem, Divider} from '@tremor/react';
import { 
  CpuChipIcon, SpeakerWaveIcon, ChatBubbleBottomCenterIcon, 
  DocumentTextIcon, PhoneArrowUpRightIcon, ClockIcon
...
const ConfigurationTab: React.FC<ConfigurationTabProps> = ({ assistant }) => {
  ...
  const modelProvider = typeof assistant.model === 'object' 
    ? assistant.model?.provider 
    : modelName?.startsWith('gpt') ? 'OpenAI' : 
      modelName?.startsWith('claude') ? 'Anthropic' : 
      modelName?.startsWith('gemini') ? 'Google' : 'Inconnu';

  // Récupérer les détails de la voix  
  const voiceId = typeof assistant.voice === 'string' 
    ? assistant.voice 
    : assistant.voice?.voiceId;
  
  const voiceProvider = typeof assistant.voice === 'object' 
    ? assistant.voice?.provider 
    : voiceId?.startsWith('eleven') ? 'ElevenLabs' :
      voiceId?.startsWith('azure') ? 'Azure' : 'Inconnu';
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <Grid numItems={1} numItemsSm={2} className="gap-6">
          <Col>
            ...
            <Metric className="mt-1 text-base">{displayValue(modelProvider)}</Metric>
          </Col>
          <Col>
            <Text className="text-gray-500">Modèle</Text>
            <Metric className="mt-1 text-base">{displayValue(modelName)}</Metric>
          </Col>
        </Grid>
        ...
        <Grid numItems={1} numItemsSm={3} className="gap-6">
          <Col>
            ...
            <Metric className="mt-1 text-base">
              {(typeof assistant.model === 'object' ? assistant.model?.temperature : null) || 0.7}
            </Metric>
          </Col>
          <Col>
            <Text className="text-gray-500">Tokens maximum</Text>
            <Metric className="mt-1 text-base">
              {(typeof assistant.model === 'object' ? assistant.model?.maxTokens : null) || 1024}
            </Metric>
          </Col>
          <Col>
            <Text className="text-gray-500">Language</Text>
            <Metric className="mt-1 text-base">{displayValue(assistant.language)}</Metric>
          </Col>
        </Grid>
      ...
      <Card>
        ...
        <Grid numItems={1} numItemsSm={2} className="gap-6">
          <Col>
            <Text className="text-gray-500">Fournisseur</Text>
            <Metric className="mt-1 text-base">{displayValue(voiceProvider)}</Metric>
          </Col>
          <Col>
      ...
      <Card>
        ...
        <Card className="mt-2 bg-gray-50">
          <Text className="whitespace-pre-wrap">
            {(assistant.metadata as any)?.endCallMessage || 'Aucun message de fin spécifié'}
          </Text>
        </Card>
      </Card>
      ...
      <Card>
            ...
            <Flex className="mt-1 items-center gap-2">
              {(assistant.metadata as any)?.forwardingPhoneNumber ? (
                <>
                  ...
                  <Metric className="text-base">{(assistant.metadata as any).forwardingPhoneNumber}</Metric>
                </>
              ) : (
        ...
        <Card className="mt-2 bg-gray-50">
          <Text className="text-gray-500 mb-2">Ces phrases, si prononcées par l&apos;utilisateur, mettront fin à l&apos;appel</Text>
          {((assistant.metadata as any)?.endCallPhrases && Array.isArray((assistant.metadata as any).endCallPhrases) && (assistant.metadata as any).endCallPhrases.length > 0) ? (
            <List>
              {((assistant.metadata as any).endCallPhrases as string[]).map((phrase, index) => (
                <ListItem key={index}>
            ...
            <Text className="text-gray-400">Aucune phrase de fin configurée</Text>
          )}
        </Card>
      </Card>
