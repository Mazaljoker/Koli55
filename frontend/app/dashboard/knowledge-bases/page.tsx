...
import { PlusCircle, Database, FileText, Clock, MoreHorizontal } from 'lucide-react';
...
export default function KnowledgeBasesPage() {
  ...
  const fetchKnowledgeBases = async () => {
    try {
      ...
      console.error('Erreur lors du chargement des bases de connaissances:', err);
  ...
  const getStatusDisplay = (status: string) => {
    switch (status) {
      ...
      case 'processing':
        return { text: 'En indexation', className: 'bg-blue-100 text-blue-800' };
      case 'error':
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-allokoli-purple-800">Bases de connaissances</h1>
          <p className="text-allokoli-blue-700 mt-2">Gérez les données utilisées par vos assistants IA</p>
        </div>
        <Button 
          type="primary"
          icon={<PlusCircle size={16} />}
          className="bg-gradient-to-r from-allokoli-purple-600 to-allokoli-purple-700 border-none shadow-md hover:shadow-lg"
          onClick={() => message.info('Création de base de connaissances à venir')}
        >
          Nouvelle base
        </Button>
      </div>
      ...
      {error && knowledgeBases.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-8">
          {knowledgeBases.map((kb) => {
            ...
            return (
              <div key={kb.id} className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden transition-all hover:shadow-md">
                <div className="p-6">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center">
                      <div className="h-10 w-10 rounded-lg bg-allokoli-purple-100 flex items-center justify-center mr-3">
                        <Database size={20} className="text-allokoli-purple-600" />
                      </div>
                      <h3 className="text-lg font-semibold text-allokoli-purple-800">{kb.name}</h3>
                    </div>
                    <button 
                ...
                <div className="px-6 py-3 bg-gray-50 border-t border-gray-100 flex justify-between items-center">
                  ...
                  <span className="text-sm text-gray-500">{getEstimatedSize(kb.fileCount || 0)}</span>
                </div>
              </div>
            );
          })}
          ...
          <div 
            ...
            onClick={() => message.info('Création de base de connaissances à venir')}
          >
            <div className="h-12 w-12 rounded-full bg-allokoli-purple-100 flex items-center justify-center mb-4">
              <PlusCircle size={24} className="text-allokoli-purple-600" />
            </div>
            <h3 className="text-lg font-semibold text-allokoli-purple-800 mb-2">Ajouter une base</h3>
            <p className="text-center text-gray-500 text-sm">
              Créez une nouvelle base de connaissances pour enrichir vos assistants
            </p>
          </div>
        </div>
      )}
      
      <div className="bg-allokoli-blue-50 p-6 rounded-lg mt-6 border border-allokoli-blue-200">
        <h3 className="text-lg font-semibold text-allokoli-blue-800 mb-3">À propos des bases de connaissances</h3>
        <p className="text-gray-700 mb-4">
          Les bases de connaissances permettent à vos assistants d&apos;accéder à des informations spécifiques à votre entreprise.
          Vous pouvez importer des documents, des FAQ, des catalogues ou tout autre texte pertinent.
        </p>
