 ...
 */
function mapToVapiSquadFormat(squadData: any): SquadCreateParams | SquadUpdateParams {
  console.log(`[MAPPING] mapToVapiSquadFormat - Input: ${JSON.stringify(squadData, null, 2)}`);
  ...
  if (squadData.members !== undefined) {
    payload.members = squadData.members;
  }
  ...
  console.log(`[MAPPING] mapToVapiSquadFormat - Output: ${JSON.stringify(payload, null, 2)}`);
  return payload;
}
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (req.method === 'POST' && squadId && action === 'members' && !memberId) {
      console.log(`[HANDLER] POST /squads/${squadId}/members - Ajout de membres à une équipe`);
      ...
      console.log(`[VAPI_SUCCESS] Added members to squad: ${squadId}`, updatedSquad);
    ...
    if (req.method === 'DELETE' && squadId && action === 'members' && memberId) {
      console.log(`[HANDLER] DELETE /squads/${squadId}/members/${memberId} - Retrait d'un membre d'une équipe`);
      
      await vapiSquads.removeMember(squadId, memberId)
      console.log(`[VAPI_SUCCESS] Removed member ${memberId} from squad: ${squadId}`);
    ...
    if (req.method === 'GET' && squadId && !action) {
      console.log(`[HANDLER] GET /squads/${squadId} - Récupération d'une équipe`);
      ...
      try {
        ...
        console.error(`[VAPI_ERROR] GET /squads/${squadId} - Failed to retrieve:`, error);
    ...
    if (req.method === 'POST' && !squadId) {
      ...
      const squadPayload = mapToVapiSquadFormat(validatedData) as SquadCreateParams;
    ...
    if (req.method === 'PATCH' && squadId && !action) {
      console.log(`[HANDLER] PATCH /squads/${squadId} - Mise à jour d'une équipe`);
      
      const data = await req.json()
      const validatedData = validateInput(data, updateSquadSchema)
      
      // Utiliser notre fonction de mapping
      const squadPayload = mapToVapiSquadFormat(validatedData) as SquadUpdateParams;
    ...
    if (req.method === 'DELETE' && squadId && !action) {
      console.log(`[HANDLER] DELETE /squads/${squadId} - Suppression d'une équipe`);
