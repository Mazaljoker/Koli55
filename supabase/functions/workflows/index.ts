 ...
 */
function mapToVapiWorkflowFormat(
  workflowData: any
): WorkflowCreateParams | WorkflowUpdateParams {
  console.log(
    `[MAPPING] mapToVapiWorkflowFormat - Input: ${JSON.stringify(
      workflowData,
      null,
      2
    )}`
  ...
  console.log(
    `[MAPPING] mapToVapiWorkflowFormat - Output: ${JSON.stringify(
      payload,
      null,
      2
    )}`
...
function mapToVapiExecuteFormat(executeData: any): WorkflowExecuteParams {
  console.log(
    `[MAPPING] mapToVapiExecuteFormat - Input: ${JSON.stringify(
      executeData,
      null,
      2
    )}`
  ...
  console.log(
    `[MAPPING] mapToVapiExecuteFormat - Output: ${JSON.stringify(
      payload,
      null,
      2
    )}`
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (req.method === "POST" && workflowId && action === "execute") {
      console.log(
        `[HANDLER] POST /workflows/${workflowId}/execute - Exécution d'un workflow`
      );
    ...
    if (req.method === "GET" && !workflowId) {
      ...
      if (isTestMode) {
        return new Response(
          JSON.stringify({
            success: true,
            message: "Workflows function deployed successfully!",
            endpoints: {
              GET_ALL: "/workflows",
      ...
      console.log(
        `[VAPI_SUCCESS] Listed ${workflows.data?.length || 0} workflows`
      );
      return new Response(
        JSON.stringify({
          data: workflows.data,
    ...
    if (req.method === "GET" && workflowId && !action) {
      console.log(
        `[HANDLER] GET /workflows/${workflowId} - Récupération d'un workflow`
      );
      ...
      try {
        ...
        console.log(
          `[VAPI_SUCCESS] Retrieved workflow: ${workflowId}`,
          workflow
        );
      } catch (error) {
        console.error(
          `[VAPI_ERROR] GET /workflows/${workflowId} - Failed to retrieve:`,
          error
        );
    ...
    if (req.method === "POST" && !workflowId) {
      ...
      // Utiliser notre fonction de mapping
      const workflowPayload = mapToVapiWorkflowFormat(
        validatedData
    ...
    if (req.method === "PATCH" && workflowId && !action) {
      console.log(
        `[HANDLER] PATCH /workflows/${workflowId} - Mise à jour d'un workflow`
      );
    ...
    if (req.method === "DELETE" && workflowId && !action) {
      console.log(
        `[HANDLER] DELETE /workflows/${workflowId} - Suppression d'un workflow`
      );
