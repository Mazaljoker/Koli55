...
async function getVapiApiKey(req: Request, requestBody: any, supabaseClient: SupabaseClient): Promise<string | undefined> {
  ...
  const apiKeyEnv = typeof Deno !== 'undefined' ? Deno.env.get('VAPI_API_KEY') : undefined;
  if (apiKeyEnv) {
  ...
  try {
    ...
    console.warn('[VAPI_KEY] Error fetching from config table:', error?.message || 'Unknown error');
...
// Fonction pour faire des requêtes à l'API Vapi - pour la rétrocompatibilité, // idéalement remplacer progressivement par les appels de vapiAssistantsasync function callVapiAPI(endpoint: string, apiKey: string, method: string = 'GET', body?: any) {  // Correction: API Vapi utilise directement les endpoints sans préfixe /v1/  const sanitizedEndpoint = endpoint.startsWith('/') ? endpoint.substring(1) : endpoint;  const url = `https://api.vapi.ai/${sanitizedEndpoint}`;
...
function getSupabaseClient(req: Request): SupabaseClient {
  ...
  // @ts-ignore - Deno existe dans l'environnement d'exécution
  const isDevelopment = (typeof Deno !== 'undefined' ? Deno.env.get('SUPABASE_ENV') : undefined) === 'development' || true;
  ...
  try {
    ...
    console.warn(`[JWT_VALIDATION] Error validating JWT: ${error?.message || 'Unknown error'}`);
    if (isDevelopment) {
      console.log('[DEV_MODE] Using development authentication despite JWT validation error.');
    } else {
      throw new Error(`JWT validation error: ${error?.message || 'Unknown error'}`);
    }
  }
...
function extractUserFromJWT(token: string): { sub: string; role: string; email?: string } | null {
  try {
    ...
    console.warn(`[JWT_EXTRACTION] Error extracting user from JWT: ${error?.message || 'Unknown error'}`);
    return null;
  }
}
        ...
        try {
          ...
          try {
            ...
            if (isDevelopment) {
              ...
              return createVapiErrorResponse(
                'Assistant created in database, but Vapi API call failed: ' + (vapiError.message || 'Unknown error'),
                500
              );
            }
          }
          ...
          return createVapiErrorResponse(error.message || 'Failed to process assistant creation', 500);
        }
      }
