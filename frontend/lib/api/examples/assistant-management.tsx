...
export default function AssistantManagement({ authToken }: AssistantManagementProps) {
  // État local
  ...
  const handleEdit = (assistant: Assistant) => {
    ...
    form.setFieldsValue({
      ...
      model: typeof assistant.model === 'string' ? assistant.model : assistant.model.model,
      modelProvider: typeof assistant.model === 'object' ? assistant.model.provider : 'openai',
      voice: typeof assistant.voice === 'string' ? assistant.voice : assistant.voice.voiceId,
      voiceProvider: typeof assistant.voice === 'object' ? assistant.voice.provider : 'eleven-labs',
    });
    setModalVisible(true);
  };
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      const assistantData: AssistantCreate | AssistantUpdate = {
        name: values.name,
        model: values.modelProvider && values.modelProvider !== 'simple' 
          ? {
          ...
          : values.model,
        voice: values.voiceProvider && values.voiceProvider !== 'simple'
          ? {
      ...
      if (modalMode === 'create') {
        ...
        await sdk.updateAssistant(selectedAssistant!.id, assistantData as AssistantUpdate);
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'model',
      key: 'model',
      render: (model: any) => typeof model === 'string' ? model : `${model.provider}/${model.model}`,
    },
    {
      title: 'Langue',
      dataIndex: 'language',
  ...
  return (
    <div className="p-6">
      ...
      <Modal
        title={
          modalMode === 'create' ? 'Nouvel Assistant' :
          modalMode === 'edit' ? 'Modifier l\'Assistant' :
      ...
      >
        {modalMode === 'view' && selectedAssistant ? (
          ...
          <div className="space-y-4">
            ...
            <div><strong>ID:</strong> {selectedAssistant.id}</div>
            <div><strong>Modèle:</strong> {JSON.stringify(selectedAssistant.model, null, 2)}</div>
            <div><strong>Voix:</strong> {JSON.stringify(selectedAssistant.voice, null, 2)}</div>
            <div><strong>Langue:</strong> {selectedAssistant.language}</div>
            <div><strong>Premier message:</strong> {selectedAssistant.firstMessage}</div>
            <div><strong>Instructions:</strong> {selectedAssistant.instructions}</div>
            <div><strong>Créé le:</strong> {new Date(selectedAssistant.created_at).toLocaleString('fr-FR')}</div>
          </div>
        ) : (
